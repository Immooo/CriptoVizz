...
3. **RabbitMQ et queues** : broker qui tamponne les événements dans `raw_news` et
   `enriched_news`. L'exchange utilisé est l'exchange direct par défaut.
...
2. RabbitMQ Management : montrer `raw_news`, `enriched_news` et leurs consumers.
...
healthchecks MySQL/RabbitMQ, variables `.env`, restart policies.
